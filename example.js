...
export default function App() {
  ...
  const [prixTrajetUnitaire, setPrixTrajetUnitaire] = useState(70);

  // Paramètres Logement
  const [prixNuitHotel, setPrixNuitHotel] = useState(120);
  ...
  const [chargesAnnuellesAchat, setChargesAnnuellesAchat] = useState(2000);
  ...
  const [taxeHabitationAnnuelle, setTaxeHabitationAnnuelle] = useState(1200); 
  ...
  const [fraisInstallation, setFraisInstallation] = useState(5000);
  ...
  const scenarios = useMemo(() => {
    ...
    // 2. Hôtel
    const coutTransportHebdoAnnuel = (prixTrajetUnitaire * 2) * arParSemaine * semainesParAn;
    const coutTransportHotelTotal = calculerTotalInflate(coutTransportHebdoAnnuel, dureeAnnees, inf);
    ...
    return [
      createScenario('tgv', 'TGV Quotidien', <Train className="w-5 h-5" />, 'sky', coutTgvQuotidien + coutNavigoTotal, 0, 0, 0, 0, 0, 0, 0, 0, "Abonnement illimité.", trajetsParAnTgv),
    ...
    abonnementTgvMensuel, prixTrajetUnitaire, prixNuitHotel, loyerMensuel,
          ...
          <aside className="lg:col-span-4 space-y-6">
            <section className="bg-white p-5 rounded-3xl shadow-sm border border-slate-200">
              <h2 className="text-sm font-black uppercase tracking-widest text-slate-400 mb-4 flex items-center gap-2">
                <Calculator className="w-4 h-4" /> Rythme & Transport
              </h2>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-500 uppercase">Abo. TGV (€/mois)</label><input type="number" value={abonnementTgvMensuel} onChange={e => setAbonnementTgvMensuel(Number(e.target.value))} className="w-full p-2 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-500 uppercase">Billet Unité (€)</label><input type="number" value={prixTrajetUnitaire} onChange={e => setPrixTrajetUnitaire(Number(e.target.value))} className="w-full p-2 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
                <div className="space-y-1 col-span-2"><label className="text-[10px] font-bold text-slate-500 uppercase">Navigo (€/mois)</label><input type="number" value={passLocalMensuel} onChange={e => setPassLocalMensuel(Number(e.target.value))} className="w-full p-2 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
              </div>
            </section>
            ...
            <section className="bg-amber-50 p-5 rounded-3xl shadow-sm border border-amber-100">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="space-y-1"><label className="text-[10px] font-bold text-amber-700 uppercase">Nuit Hôtel (€/j)</label><input type="number" value={prixNuitHotel} onChange={e => setPrixNuitHotel(Number(e.target.value))} className="w-full p-2 bg-white/50 border-none rounded-xl focus:ring-2 focus:ring-amber-500 outline-none" /></div>
                <div className="space-y-1"><label className="text-[10px] font-bold text-amber-700 uppercase">Loyer Paris (€/mois)</label><input type="number" value={loyerMensuel} onChange={e => setLoyerMensuel(Number(e.target.value))} className="w-full p-2 bg-white/50 border-none rounded-xl focus:ring-2 focus:ring-amber-500 outline-none" /></div>
                <div className="space-y-1"><label className="text-[10px] font-bold text-amber-700 uppercase">Repas Hôtel (€/j)</label><input type="number" value={budgetRepasHotelJour} onChange={e => setBudgetRepasHotelJour(Number(e.target.value))} className="w-full p-2 bg-white/50 border-none rounded-xl focus:ring-2 focus:ring-amber-500 outline-none" /></div>
                <div className="space-y-1"><label className="text-[10px] font-bold text-amber-700 uppercase">Repas Appart (€/j)</label><input type="number" value={budgetRepasAppartJour} onChange={e => setBudgetRepasAppartJour(Number(e.target.value))} className="w-full p-2 bg-white/50 border-none rounded-xl focus:ring-2 focus:ring-amber-500 outline-none" /></div>
              </div>
                      ...
                      <div className="flex justify-between items-end mb-1">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className={`text-[10px] font-bold ${s.id === 'tgv' && s.trajetsParAn > 250 ? 'text-red-500' : 'text-slate-400'}`}>
                              {s.trajetsParAn} trajets / an
                              {s.id === 'tgv' && s.trajetsParAn > 250 && ' ⚠️ (Forfait de 250 dépassé)'}
                            </span>
                          </div>
                        </div>
                  ...
                  <tr className="bg-slate-50/50">
                    ...
                    {scenarios.map(s => (
                      <td key={`tj-${s.id}`} className="p-3 text-center">
                        <span className={`px-2 py-1 rounded-md font-bold text-[11px] ${s.id === 'tgv' && s.trajetsParAn > 250 ? 'bg-red-100 text-red-700' : 'bg-sky-100 text-sky-700'}`}>
                          {s.trajetsParAn} {s.id === 'tgv' && s.trajetsParAn > 250 && '⚠️'}
                        </span>
                      </td>
                    ))}
                  </tr>
